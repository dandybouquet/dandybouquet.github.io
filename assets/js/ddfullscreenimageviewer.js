...
;(function($){
    ...
    var $loadingdiv = null
    var canvashtml = '<div id="fullscreencanvas"><div id="fullscreenimagearea"></div></div><div id="fullimageloadingdiv"></div><div id="closeviewer" title="Close Viewer">Close</div>'
    ...
    function fullscreenimage($img, setting){ // fullscreenimage plugin function
        if (!$canvasref){
            ...
            $fullscreenimagearea.on('click', function(){
                $(document.body).removeClass('revealviewer')
                $fullscreenimagearea.data('$largeimage').unbind()
                $fullscreenimagearea.empty()
                $loadingdiv.css('visibility', 'hidden')
        ...
        $img.on('click', function(){
            ...
            $largeimage.on('load', function(){ // whenlarge image loads
                ...
                $fullscreenimagearea.html( '<img class="zoomed-img" src="' + largeimage.src + '" style="width:' + largeimage.width*scale + 'px; height:' + largeimage.height*scale + 'px;" />' )
                $loadingdiv.css('visibility', 'hidden')
            })
